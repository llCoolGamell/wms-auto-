...
def app_dir():
    """Папка скрипта или exe (для логов и автозагрузки зон)."""
...
#  Logging
# ---------------------------------------------------------------------------
_log_path = os.path.join(app_dir(), "wms_auto.log")
logging.basicConfig(
    level=logging.DEBUG,
...
def _edit_on_same_row(static_ctrl, edits, row_tolerance=35):
    """Edit на одной строке со Static-меткой."""
    try:
        sr = static_ctrl.rectangle()
...
def find_labeled_edit(window, label_text, exclude_in_label=(), row_tolerance=35):
    """
    Находит Edit рядом с Static-меткой, содержащей label_text.
    exclude_in_label — подстроки в метке, при которых строка пропускается.
    """
    statics = get_children_by_class(window, "Static")
    edits = get_children_by_class(window, "Edit")
    label_lower = label_text.lower()
    excludes = [e.lower() for e in exclude_in_label]

    target_static = None
    for s in statics:
        try:
            txt_lower = s.window_text().strip().lower()
            if any(ex in txt_lower for ex in excludes):
                continue
            if label_lower in txt_lower:
                target_static = s
                break
        except Exception:
            continue
    ...
    if not target_static:
        return None
    return _edit_on_same_row(target_static, edits, row_tolerance)


def find_field_by_static_keywords(window, include_keywords, exclude_in_label=(),
                                  row_tolerance=35):
    """
    Ищет Edit по метке Static: должна содержать одно из include_keywords
    и не содержать exclude_in_label.
    При нескольких совпадениях берётся лучшая метка (точнее совпадение).
    """
    statics = get_children_by_class(window, "Static")
    edits = get_children_by_class(window, "Edit")
    includes = [k.lower() for k in include_keywords]
    excludes = [e.lower() for e in exclude_in_label]
    ...
    for s in statics:
        ...
        for kw in includes:
            if kw in txt_lower:
                score += 10
                if txt_lower.startswith(kw) or txt_lower.endswith(kw):
                    score += 5
        if score > best_score:
            best_score = score
            best_static = s

    if not best_static or best_score <= 0:
        return None
    return _edit_on_same_row(best_static, edits, row_tolerance)


def find_control_edit(window):
    """Поле «Контроль» — по метке или первое пустое Edit."""
    ctrl = find_labeled_edit(window, "контроль")
    if ctrl:
        return ctrl
    for ed in get_edits(window):
        try:
            if not ed.window_text().strip():
                return ed
        except Exception:
            continue
    edits = get_edits(window)
    return edits[-1] if edits else None


def get_pallet_edit_and_value(window):
    """
    Значение только из поля «Паллета», не из «Из места» / «Назначение».
    """
    reject = ["из места", "из груза", "назначение", "контроль", "зона"]
    for keywords in (["паллета"], ["палета"], ["паллет"]):
        ed = find_field_by_static_keywords(window, keywords, reject)
        if ed:
            val = ed.window_text().strip()
            if val:
                return ed, val

    place_ed = find_labeled_edit(window, "из места")
    place_val = ""
    if place_ed:
        try:
            place_val = place_ed.window_text().strip()
        except Exception:
            pass
    ...
    for ed in get_edits(window):
        try:
            ...
            continue
        if val and val != place_val:
            filled.append((ed, val))
...
class WMSAutomation:
    ...
    def handle_move_to_source(self, w):
        ...
        # «Из места» или «Место» (не паллета/коробка) → «Контроль»
        source_edit = (
            find_labeled_edit(w, "из места")
            or find_labeled_edit(
                w, "место",
                exclude_in_label=["паллет", "коробк", "назнач", "контроль", "зона"]
            )
        )
        control_edit = find_control_edit(w)
    ...
    def handle_search_pallet(self, w):
        """
        v3.1: только поле «Паллета» → «Контроль».
        Не используем порядок Edit (иначе подставляется «Из места»).
        """
        self.log("→ Поиск палеты")
        edit_vals = [e.window_text().strip() for e in get_edits(w)]
        self.log("   Поля: {}".format(edit_vals))

        pallet_edit, pallet_val = get_pallet_edit_and_value(w)
        control_edit = find_control_edit(w)

        if not pallet_val:
            self.log("   Поле «Паллета» пустое или не найдено!", "WARNING")
            return
        if not control_edit:
            self.log("   Поле «Контроль» не найдено!", "WARNING")
            return
    ...
    def handle_search_box(self, w):
        ...
        # «Коробка» / «Из груза» → «Контроль»
        box_edit = (
            find_field_by_static_keywords(w, ["коробка"], ["контроль", "назнач", "зона"])
            or find_labeled_edit(w, "из груза")
            or find_labeled_edit(w, "коробк", exclude_in_label=["контроль"])
        )
        control_edit = find_control_edit(w)
...
class AppGUI:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("WMS Automator v{}".format(APP_VERSION))
